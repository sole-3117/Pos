...
export const requireAuth = (req: Request, res: Response, next: NextFunction) => {
  ...
  // Agar upstream middleware allaqachon foydalanuvchini o'rnatgan bo'lsa
  ...
  // 1. Agar maxsus headerlar orqali uzatilgan bo'lsa
  ...
  // 2. Bearer token mavjud bo'lsa (sodda / JWT dekodlash mock/base64 yoki token)
  if (authHeader && authHeader.startsWith('Bearer ')) {
    const token = authHeader.substring(7);
    try {
      // Base64 yoki JSON payload bo'lsa dekodlaymiz
  ...
  // Agar kassa/web sessiya cookie yoki so'rovi bo'lsa (development uchun xavfsiz fallback)
